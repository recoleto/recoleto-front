...
export const companySchema = object({
    fantasyName: string().required('Nome Fantasia é obrigatório.'),
    ...
    telNumber: string().required('Telefone é obrigatório.').matches(/^[0-9]{10,11}$/, 'Telefone inválido.'),
    email: string().email().required('Email é obrigatório.').matches(/^[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$/, 'Email inválido.'),
    password: string().required('Senha é obrigatória.').matches(/^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$/),
